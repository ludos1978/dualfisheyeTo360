...
def multi_band_blending(img1, img2, overlap_w, leveln=None, flag_half=False):
    ...
    subASave = (subA).astype('uint8')
    subBSave = (subB).astype('uint8')
    maskSave = (mask*128).astype('uint8')
    from PIL import Image
    Image.fromarray(subASave).save("subASave.png")
    Image.fromarray(subBSave).save("subBSave.png")
    Image.fromarray(maskSave).save("maskSave.png")
